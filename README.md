...
Trong những năm gần đây, các mô hình học sâu đã giúp cải thiện đáng kể độ chính xác của OCR, đặc biệt với các ngôn ngữ có dấu như tiếng Việt.
Trong đồ án này, nhóm tập trung nghiên cứu, huấn luyện và đánh giá mô hình VietOCR nhằm nhận diện văn bản tiếng Việt từ ảnh, đồng thời so sánh hiệu quả giữa mô hình fine-tune và mô hình pre-trained gốc.
2. Mục tiêu
...
3.2 Tesseract OCR
Trong đồ án này, Tesseract không được dùng để nhận diện ký tự, mà được sử dụng để:
...
| Pre-trained | ~7.07% |
| Fine-tuned  | ~6.59% |
Kết quả cho thấy mô hình huấn luyện lại cải thiện độ chính xác, đặc biệt với các từ tiếng Việt có dấu và từ đặc thù.
8. Triển khai (Inference)
Giao diện đơn giản sử dụng Tkinter
Thư viện: Pillow, pytesseract, VietOCR
Các bước:
Load ảnh
Tiền xử lý (grayscale, threshold, resize)
Tách dòng bằng Tesseract
Nhận diện từng dòng bằng VietOCR
